...
當使用者在 Fooday 地圖上建立地點時，餐廳將立即出現在地圖上，建立者能立刻獲得 Fudos，其他使用者也能即時在該地點留下評論。然而，為了確保準確性和可靠性，該地點將進入為期 7 天的待審期。

在此期間，使用者可以隨時檢舉，並可以重複建立相同的地點。但若最初建立的地點沒有被檢舉，又或檢舉失敗，後續建立的相同地點將不會獲得 Fudos，且會從平台上移除。
...
在檢舉提交後，Fooday的投票系統將啟動，該系統會隨機挑選多名使用者參與投票。這些被隨機選中的使用者將仔細審查被檢舉的餐廳資訊或評論，並根據自己的判斷投下「同意」或「不同意」的票。
